...
### 0. 核心要求
- 目标不是“走错以后容易修”，而是**让 Agent 第一次就沿着正确主路径完成任务**
- 设计 Skill / 脚本时，优先追求**脚本直接引导正确生成**，而不是“模型先自由发挥，再靠校验器拦截”
- **大模型尽可能少参与流程执行**；它的上下文应更多留给业务内容、证据、权衡和技术结论，而不是命令顺序、票据流转、修复路径
- 一句话原则：**脚本出卷，模型填空，脚本成稿**

### 0.1 脚本驱动优先于校验驱动
- 优先做成“脚本定义流程 + 脚本生成结构 + 模型填写内容”的体系
- 校验器仍然需要存在，但它的职责是兜底，不应成为主流程的核心引导手段
- 如果一个流程主要依赖模型记住步骤、拼接命令、推断格式，再由脚本事后拦截，这说明架构仍然偏“校验驱动”

### 0.2 大模型尽可能少参与流程执行
- 不要让模型承担本可由脚本承担的流程细节，例如：
  - 何时 `--prepare`
  - 何时 `--mark-step-card-read`
  - ticket 如何流转
  - 状态文件如何迁移
  - 失败后该回滚到哪一步
  - 该使用哪个底层 flag
- 这些知识一旦占据模型上下文，就会挤压业务分析和内容生成的注意力
...
### 0.4 结构化优先于自由文本
- 只要可以结构化，就不要让模型自由写大段格式文本
- 优先采用 JSON / YAML / schema / 表单字段，让模型提供“业务值”而不是“最终排版”
- Markdown 更适合作为脚本渲染产物，而不是模型的原始提交格式

### 0.5 模板与状态真相源必须在脚本侧
- 槽位列表、模板结构、票据、工作流状态，都必须以脚本计算结果为准
- 不要暴露一个可被模型自由改写的 state 真相源，尤其是 `slots`、slot count、ticket 状态、模板指纹等关键流程字段
- 模型可以补充内容，但不应主导流程骨架

### 0.6 发现问题时先反思架构，不先打补丁
- 当模型反复在某一步走错时，优先问：
  1. 是否仍然存在多个公开入口，而不是单一高层入口？
  2. 输出是否仍然过于自由，导致模型要自己猜格式？
  3. 状态真相是否暴露给模型，导致它能手改流程骨架？
  4. 文档最终形态是否仍然主要依赖模型手写，而不是脚本渲染？
- 如果答案是“是”，优先收敛架构边界，而不是继续追加 validator 补丁

### 0.7 不只修当前暴露点，要追查同类风险面
- 不要因为这次只在某一个步骤暴露错误，就只分析这一个步骤
- 必须追问：这个错误是该步骤独有，还是整条流程里一类同构问题的首次暴露
- 其他步骤即使这次没有失败，也可能共享同样的结构缺陷，例如：
  - 同样依赖模型手写格式
  - 同样暴露了可手改的状态真相源
  - 同样存在多个公开入口
  - 同样把 repair path 暴露给模型
- 因此修复时优先看“错误类型”而不是“报错步骤编号”，优先消灭一类问题，而不是只扑灭当前症状
