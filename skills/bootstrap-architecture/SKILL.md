...
- 操作：
  - 必须询问用户："是否要自定义技术方案模板？"
  - 如果用户明确表示要自定义：调用 `manage-technical-solution-template` 技能
  - 如果用户明确表示不要自定义：如有需要创建 `.architecture/templates/` 目录，将默认模板复制到 `.architecture/templates/technical-solution-template.md`
