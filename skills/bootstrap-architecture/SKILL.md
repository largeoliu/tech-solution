...
- 每个步骤完成后写入独立的 `checkpoints.step-<N>.yaml` 文件，而非 `current.yaml` 的 `checkpoints` 字段
- 遇到 STOP_AND_ASK：设置 `blocked: true` 并填写 `block_reason`
- 步骤 4 负责创建 `.architecture/templates/` 并写入模板，会询问用户选择默认模板或提供自定义模板

## 回退规则

| 触发条件 | 回退到 | 影响范围 |
|----------|--------|----------|
| 项目基础上下文重大变化（框架迁移/目录重构等） | 步骤 1 | 全部产物需重新验证 |
...
1. 步骤 4 会询问用户选择默认模板或自定义模板；各步骤门控 STOP_AND_ASK 触发时，向用户提问获取缺失信息后继续执行。
2. 如需在初始化完成后替换模板，可显式调用 `manage-technical-solution-template`。
...
| 4 | `steps/4-confirm-template.md` | 确认技术方案模板并完成初始化 |
