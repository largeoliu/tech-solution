...
- 创建 `.architecture/` 目录结构（按需创建）。
- 生成 `.architecture/members.yml`、`.architecture/principles.md`（按需生成定制后的文件）。
- 生成 `.architecture/templates/technical-solution-template.md`（按需复制或写入）。
...
- 从模板中识别项目需要的成员，添加模板中不存在的成员（如需要），直接生成定制后的 `.architecture/members.yml`（同时创建 `.architecture/` 目录）。
- 不预先复制模板文件。
- 未完成第 2 步，不得进入第 3 步。
...
- 直接根据模板定制生成 `.architecture/principles.md`，不预先复制模板文件。
- 未完成第 3 步，不得进入第 4 步。
...
- 若回答"不需要"或明确表示保留当前模板，按需创建 `.architecture/templates/` 目录，复制默认模板到 `.architecture/templates/technical-solution-template.md`。
