...
- 任一步骤只有在状态文件门禁字段允许时才能进入下一步；不得以口头判断替代状态门禁
...
- 凡 `required_artifacts` 未齐、`blocked = true`、`absorption_check_passed = false`、存在未解决阻塞槽位，或任一步骤被跳过时，禁止清理 working draft 和状态文件
