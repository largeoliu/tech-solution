...
- 主 skill 负责唯一主路径、关键停机规则、模板槽位摘要、最小质量门槛和完成标准；引用文档负责完整细则。
- 只在需要正式技术方案文档时使用；如果只是初始化 `.architecture/`、补跑初始化或替换模板，转到 `bootstrap-architecture`。

## 主文档可见的最小信息块

最小信息块严格由当前生效模板决定。读取当前 `.architecture/templates/technical-solution-template.md` 后，根据模板的章节结构识别出信息块清单，作为后续分析、收敛和成稿的唯一正文骨架。
...
任一缺失时立即停止，明确说明初始化未完成，并引导用户先使用 `bootstrap-architecture`。

**未完成第 2 步，不得进入第 3 步。** 语义前置文件缺失时必须停在第 2 步并引导用户运行 `bootstrap-architecture`，不能跳过到模板读取。
...
质量门槛与内容完整性检查见 [references/solution-process.md](references/solution-process.md)。
...
5. 再生成 `模板任务单`。
6. 每个阶段的产物完成后，必须立即写入 working draft 的对应区块，然后立即在对话中展示该产物的摘要，然后才能进入下一阶段。不允许跳过写入或展示，也不允许把多个阶段的产物合并到一起再批量写入或展示。具体顺序：
   - 第 6 步完成 → 写入 WD-CTX → 展示共享上下文清单摘要 → 进入第 7 步
   - 第 7 步完成 → 写入 WD-TASK → 展示模板任务单摘要 → 进入第 8 步
   - 第 8 步每个成员完成 → 写入 WD-EXP-[expert-slug] → 展示该专家产物摘要
   - 第 9 步完成 → 写入 WD-SYN → 展示协作收敛纪要摘要 → 进入第 10 步
...
10. 不允许在跳过中间产物生成和展示的情况下直接输出最终技术方案文档。如果最终文档中的内容无法追溯到 working draft 中已写入的 WD-CTX、WD-TASK、WD-EXP-*、WD-SYN 区块，则该文档无效。
