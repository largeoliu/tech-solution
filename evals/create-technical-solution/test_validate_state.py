...
class TestScripts:
    ...
    def test_mark_step_skipped_removes_completed_step(self, workspace: dict[str, Path]) -> None:
        state = make_state(workspace, current_step=9, completed_steps=[1, 2, 3, 4, 5, 6, 7, 8, 9], skipped_steps=[])
        write_state(workspace, state)
