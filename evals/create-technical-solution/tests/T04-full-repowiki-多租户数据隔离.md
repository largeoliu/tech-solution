...
2. 步骤 6 检测并记录 repowiki 路径，后续分析引用其中内容
3. 专家分析的关键结论仍绑定 CTX 编号，repowiki 仅作补充
4. 每个需要专家判断的槽位均有对应的 WD-EXP-* 区块
5. 每位专家仅对参与槽位写判断，参与槽位字段非全部
6. WD-SYN 按槽位逐个写入，checkpoint 展示逐槽位收敛过程
7. 若结论无法在现有模板承载，记录模板承载缺口并阻塞
8. 最终文档一级章节与模板完全一致
...
| A04.6 | skill-revealing | 模板承载缺口记录 | 缺失时 blocked=true | 未记录 |
