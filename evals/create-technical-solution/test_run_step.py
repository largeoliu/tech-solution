...
class TestRunStepBehavior:
    ...
    def test_light_sample_flow_end_to_end(self, workspace: dict[str, Path]) -> None:
        ...
        assert (
            run_step.complete_step(
                make_args(
                    workspace["state_path"], "WD-CTX 完成", stdin_content=WD_CTX_BODY
                )
            )
            == 0
        )
        assert (
            run_step.complete_step(
                make_args(
                    workspace["state_path"], "WD-TASK 完成", stdin_content=WD_TASK_BODY
                )
            )
            == 0
        )
        ...
        assert (
            run_step.complete_step(
                make_args(
                    workspace["state_path"], "专家分析完成", stdin_content=exp_blocks
                )
            )
            == 0
        )
        assert (
            run_step.complete_step(
                make_args(
                    workspace["state_path"], "WD-SYN 完成", stdin_content=syn_blocks
                )
            )
            == 0
        )
        ...
        assert (
            run_step.complete_step(make_args(workspace["state_path"], "成稿完成")) == 0
        )
        assert (
            run_step.complete_step(make_args(workspace["state_path"], "清理完成")) == 0
        )
